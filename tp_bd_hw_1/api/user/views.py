...
def create(request):
    ...
    try:
        user_qs = __cursor.execute(create_user_query, [username, about, name, email])
...
def details(request):
    ...
    # validate user
    try:
        user_id_qs = __cursor.execute(get_user_by_email_query, [email, ]).fetchone()
    except DatabaseError as db_err: 
        __cursor.close()
        return HttpResponse(dumps({'code': codes.UNKNOWN_ERR,
...
def updateProfile(request):
    ...
    try:
        user_qs = __cursor.execute(update_profile_query, [about, name, email, email])
    except DatabaseError as db_err: 
        __cursor.close()
        return HttpResponse(dumps({'code': codes.UNKNOWN_ERR,
                                   'response': str(db_err)}))
